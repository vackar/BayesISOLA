...
class mouse:
	...
	def fit_mouse(self, st, t_min=0, t_max=0):
		...
		if (t_min <= 0 or t_min/dt > length):
			print ('Warning: param t_min={0:4.2f} point out of the record. Changing to t_min=10.'.format(t_min))
			t_min = 10
		if (t_max <= 0 or t_max/dt > length):
			print ('Warning: param t_max={0:4.2f} point out of the record. Changing to t_max={1:4.2f}.'.format(t_min, length*dt-10))
			t_max = length*dt-10
		if (self.fit_time_before > t_min):
			print ('Warning: param fit_time_before={0:4.2f} would result in fitting out of the traces. Changing to fit_time_before={1:4.2f}.'.format(self.fit_time_before, t_min))
			self.fit_time_before = t_min
		if (self.fit_time_after > length*dt-t_max):
			print ('Warning: param fit_time_after={0:4.2f} would result in fitting out of the traces. Changing to fit_time_after={1:4.2f}.'.format(self.fit_time_after, length*dt-t_max))
			self.fit_time_after = length*dt-t_max
	...
	def invert(self, N, E, Z, shift):
		...
		for i in range(fit_t0, fit_t1):
			try: # FIXME
				...
				print (i+shift-on, 'in', len(mys)) # FIXME
				print (i, 'in', len(N)) # FIXME
				print (fit_t0, fit_t1)
				exit() # FIXME
	...
	def invert_1D(self, D, shift):
		...
		:type shift: integer
		:param shift: time shift between synthetic mouse and real record in samples; shift=0 means the mouse onset is at the end of the record, shift>0 shifts mouse to earlier time
		"""
		dt = self.dt; mys = self.mouse
		t_mouse = (self.reclen - shift) * dt
		fit_t0 = int((t_mouse - self.fit_time_before) / dt)
